...
<a href="<c:if test="${href=='#'}">#</c:if><c:if test="${href!='#'}"><c:url value="/${dir}/${href}" /></c:if>" <t:attr attr="xclass" value="${xclass}"/><t:attr attr="style" value="${style}"/><t:attr attr="target" value="${target}"/>><jsp:doBody /></a>
